...
<script>
  ...
  export default {
    ...
    methods: {
      //进入页面自动触发展示用户列表
      loadData() {
        var params = {personName: this.query.personName};
        this.$api.get('list',params, result => {
          console.log(result)
          this.userInfoList = result.result;
        })
      },
      ...
      deletePerson(rowData) {
        this.$alert('是否删除此用户', '信息删除', {
          ...
          callback: action => {
            ...
            this.$api.delete('delete',params, result => {
              console.info(result);
              if (result.success) {
                this.$message({
                  type: 'info',
      ...
      addSubmit() {
        ...
        this.$refs.addFormData.validate((valid) => {
          //代表通过验证 ,将参数传回后台
          if (valid) {
            var params = Object.assign({}, this.addFormData);
            this.$api.post('submit',params, result => {
              if (result.success) {
                this.$message({
                  type: 'info',
                  message: '添加成功',
